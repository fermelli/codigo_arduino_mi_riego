...
void lecturasSensores(int numeroLecturas)
{
    ...
    String marcaDeTiempo = ahora.timestamp();
    ...
    if (millis() - tiempoMilisegundos >= tiempoEntreGuardadoDeLecturasRegistradas || tiempoMilisegundos == 0)
    {
        ...
        if (miArchivo)
        {
            if (lecturasSensorHRTSonValidas)
            {
                String registro = String(promedioPorcentajesSHS1) + "," + String(promedioPorcentajesSHS2) + "," + String(humedadRelativa) + "," + String(temperaturaCelcius) + "," + marcaDeTiempo;
